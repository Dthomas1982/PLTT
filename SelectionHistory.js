...
function getSelectionHistoryFiltered(playerID, gameweekID) {
  playerID = String(playerID || '').trim();
  ...
  const validPlayers = {};
  players.forEach(function(player) { validPlayers[player.playerID] = player; });

  if (playerID && !validPlayers[playerID]) return errorResponse('Selected player is not available.');
  if (gameweekID && !gameweeks.some(function(gw) { return gw.gameweekID === gameweekID; })) return errorResponse('Selected Gameweek is not available.');

  const targetGameweeks = gameweekID ? gameweeks.filter(function(gw) { return gw.gameweekID === gameweekID; }) : gameweeks;
  const targetPlayers = playerID ? [validPlayers[playerID]] : players;
  const groups = [];
  const playerOptions = players.slice();

  targetGameweeks.forEach(function(gameweek) {
    const fixtureData = getSelectionHistoryFixtures_(gameweek.gameweekID);
    const fixtureMap = {};
    fixtureData.forEach(function(fixture) { fixtureMap[String(fixture.matchID)] = fixture; });
    ...
    targetPlayers.forEach(function(player) {
      ...
      const completedFixtures = fixtures.filter(function(fixture) { return fixture.actualHome !== null && fixture.actualAway !== null; }).length;
      const totalPoints = fixtures.reduce(function(total, fixture) { return total + (fixture.points === null ? 0 : Number(fixture.points)); }, 0);
      groups.push({
  ...
  return successResponse('Selection History Loaded', {
    ...
    selectedGameweekID: gameweekID,
    groups: groups,
    totalGroups: groups.length,
    totalPoints: groups.reduce(function(total, group) { return total + Number(group.totalPoints || 0); }, 0)
  });
}

function testPlayerSelectionHistoryFiltered() {
  return getSelectionHistoryFiltered('', '');
}
